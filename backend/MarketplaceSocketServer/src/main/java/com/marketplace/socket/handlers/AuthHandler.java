...
public class AuthHandler {
    ...
    public String handleRegister(String jsonPayload) {
        try {
            ...
            if (req.username == null || req.username.isBlank()) return JsonUtil.error(400, "username is required");
            if (req.email == null || req.email.isBlank()) return JsonUtil.error(400, "email is required");
            if (req.password == null || req.password.isBlank()) return JsonUtil.error(400, "password is required");

            if (req.username.length() > 50) return JsonUtil.error(400, "username too long");
            if (req.email.length() > 100) return JsonUtil.error(400, "email too long");
            if (req.password.length() < 6) return JsonUtil.error(400, "password too short (min 6)");

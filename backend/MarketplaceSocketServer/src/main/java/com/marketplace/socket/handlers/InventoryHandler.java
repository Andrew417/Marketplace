...
public class InventoryHandler {
    ...
    public InventoryHandler(javax.sql.DataSource dataSource) {
    ...
    public String handleAddItem(String jsonPayload) {
        try {
            ...
            if (req.name == null || req.name.isBlank())
                return JsonUtil.error(400, "name is required");
            if (req.price == null)
                return JsonUtil.error(400, "price is required");
            if (req.quantity == null)
                return JsonUtil.error(400, "quantity is required");
            if (req.quantity < 0)
                return JsonUtil.error(400, "quantity must be >= 0");
    ...
    public String handleUpdateStock(String jsonPayload) {
        try {
            ...
            if (req.seller_id == null || req.seller_id.isBlank())
                return JsonUtil.error(400, "seller_id is required");
            if (req.item_id == null || req.item_id.isBlank())
                return JsonUtil.error(400, "item_id is required");
            if (req.quantity == null)
                return JsonUtil.error(400, "quantity is required");
            if (req.quantity < 0)
                return JsonUtil.error(400, "quantity must be >= 0");
